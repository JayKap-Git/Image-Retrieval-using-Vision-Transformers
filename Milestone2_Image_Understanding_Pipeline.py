...
@torch.no_grad()
def eval_report(cfg: TrainConfig, split: str = "val", max_mis: int = 16, do_tsne: bool = True, do_pca: bool = True):
    # Load everything
    vocab = Vocab.from_files(cfg.classes_txt, cfg.attributes_yaml)
    dl_tr, dl_va, dl_te = build_dataloaders(cfg, vocab)
    dl = {"train": dl_tr, "val": dl_va, "test": dl_te}[split]

    device = torch.device(cfg.device)
    run_dir = os.path.join(cfg.out_dir, os.path.basename(cfg.model_name))
    os.makedirs(run_dir, exist_ok=True)
    best_path = os.path.join(run_dir, "best.pt")
    assert os.path.exists(best_path), f"Missing best checkpoint at {best_path} (train first)."
    ...
    # Collect logits, targets, paths, and features
    logits_cls, tgts_cls, paths, feats = [], [], [], []
    attr_logits = {k: [] for k in ["color","material","condition","size"]}
    attr_tgts   = {k: [] for k in ["color","material","condition","size"]}

    for x, y, batch_paths in tqdm(dl, desc=f"report-{split}", leave=False):
        x = x.to(device)
        out = model(x)
        f = model.extract_features(x)

        logits_cls.append(out["class"].cpu())
        tgts_cls.append(y["class"])
        feats.append(f.cpu())

        for k in attr_logits: attr_logits[k].append(out[k].cpu())
        for k in attr_tgts:   attr_tgts[k].append(y[k])
        paths.extend(batch_paths)

    logits_cls = torch.cat(logits_cls, 0)
    tgts_cls   = torch.cat(tgts_cls,   0)
    feats      = torch.cat(feats,      0).numpy()
    for k in attr_logits: attr_logits[k] = torch.cat(attr_logits[k], 0)
    for k in attr_tgts:   attr_tgts[k]   = torch.cat(attr_tgts[k],   0)

    # Overall + per-class metrics
    mask = tgts_cls != -100
    y_true = tgts_cls[mask].numpy()
    y_prob = F.softmax(logits_cls[mask], dim=1).numpy()
    y_pred = y_prob.argmax(1)

    report = classification_report(y_true, y_pred, target_names=vocab.classes, output_dict=True, zero_division=0)
    with open(os.path.join(run_dir, f"class_report_{split}.json"), "w") as f:
        json.dump(report, f, indent=2)

    # Confusion matrix + plot
    cm = confusion_matrix(y_true, y_pred, labels=list(range(len(vocab.classes))))
    np.save(os.path.join(run_dir, f"confmat_{split}.npy"), cm)
    save_confmat_plot(cm, vocab.classes, os.path.join(run_dir, f"confmat_{split}.png"))

    # Misclassifications CSV + grid
    wrong = np.where(y_pred != y_true)[0]
    conf  = y_prob[np.arange(len(y_prob)), y_pred]
    order = wrong[np.argsort(-conf[wrong])]
    rows = []
    for i in order[:max_mis]:
        rows.append({
            "path": paths[i],
            "true": vocab.classes[int(y_true[i])],
            "pred": vocab.classes[int(y_pred[i])],
            "pred_conf": float(conf[i])
        })
    mis_csv = os.path.join(run_dir, f"misclass_{split}.csv")
    pd.DataFrame(rows).to_csv(mis_csv, index=False)
    save_image_grid([r["path"] for r in rows], [f"{r['pred']} ({r['pred_conf']:.2f}) vs {r['true']}" for r in rows],
                    os.path.join(run_dir, f"misclass_{split}.png"), cols=4)

    # Feature projections (TSNE/PCA)
    if do_tsne:
        save_2d_projection(feats, y_true, vocab.classes, os.path.join(run_dir, f"features_tsne_{split}.png"), method="tsne")
    if do_pca:
        save_2d_projection(feats, y_true, vocab.classes, os.path.join(run_dir, f"features_pca_{split}.png"), method="pca")

    print(f"[report] saved to {run_dir}: class_report_{split}.json, confmat_{split}.png, misclass_{split}.csv/png, features_tsne/pca_{split}.png")

def save_confmat_plot(cm: np.ndarray, class_names: List[str], out_png: str):
...
def save_2d_projection(features: np.ndarray, y: np.ndarray, class_names: List[str],
                       out_png: str, method: str = "tsne"):
    if method == "tsne":
        # TSNE is sensitive: perplexity must be < n_samples, and sklearn signatures vary by version.
        n = int(features.shape[0])
        perplexity = max(5, min(30, n - 1))
        try:
            tsne = TSNE(n_components=2, perplexity=perplexity, init="pca", random_state=42)
        except TypeError:
            # Older/newer sklearn without 'init' etc.
    ...
    scatter = ax.scatter(proj[:, 0], proj[:, 1], c=y, s=10, cmap="tab10", alpha=0.8)
    handles, _ = scatter.legend_elements(num=len(class_names))
    ax.legend(handles, class_names, title="Classes", bbox_to_anchor=(1.05, 1), loc="upper left", borderaxespad=0.)
    ax.set_title(f"Feature {method.upper()} (2D)")
    ax.set_xticks([]); ax.set_yticks([])
    plt.tight_layout()
    fig.savefig(out_png, dpi=150)
    plt.close(fig)

def save_image_grid(paths: List[str], titles: List[str], out_png: str, cols: int = 5):
    import math
    ...
    for i, p in enumerate(paths[:rows*cols]):
        ...
        axes[r, c].set_title(titles[i][:24], fontsize=8)
        axes[r, c].axis("off")
    plt.tight_layout()
    fig.savefig(out_png, dpi=150)
    plt.close(fig)
...
# -----------------------------
# Single-image prediction (class + attributes)
# -----------------------------
def predict_cmd(cfg: TrainConfig, image_paths: List[str], topk: int = 5, save_json: Optional[str] = None, res: int = 224):
    device = torch.device(cfg.device)
    vocab = Vocab.from_files(cfg.classes_txt, cfg.attributes_yaml)

    run_dir = os.path.join(cfg.out_dir, os.path.basename(cfg.model_name))
    best_path = os.path.join(run_dir, "best.pt")
    assert os.path.exists(best_path), f"Missing checkpoint: {best_path}. Train first."
    ...
    _, tfm = make_transforms(res)

    results = []
    for path in image_paths:
        full = path if os.path.isabs(path) else os.path.join(cfg.data_root, path)
        if not os.path.exists(full):
        ...
        # Class top-k
        cls_probs = F.softmax(out["class"], dim=1)[0].cpu().numpy()
        cls_topk_idx = np.argsort(-cls_probs)[:topk]
        cls_topk = [{"label": vocab.classes[i], "prob": float(cls_probs[i])} for i in cls_topk_idx]

        # Attributes (argmax + prob)
        def head_pred(head_name: str, labels: List[str]):
        ...
        pred = {
            ...
            "class": {
                ...
                "topk": cls_topk,
                "humanized": humanize_label(cls_topk[0]["label"]),
            },
            "attributes": {
        ...
        # Pretty print
        print(f"\nImage: {path}")
        print(f"Class: {pred['class']['top1']} (p={pred['class']['top1_prob']:.3f})")
        print("Top-{}:".format(topk))
        for i, item in enumerate(pred["class"]["topk"], 1):
            print(f"  {i}. {item['label']} (p={item['prob']:.3f})")
        for k in ["color","material","condition","size"]:
            a = pred["attributes"][k]
            top3_str = ", ".join(f"{t['label']}({t['prob']:.2f})" for t in a['top3'])
            print(f"{k}: {a['label']} (p={a['prob']:.3f}) | top3: {top3_str}")

    if save_json and results:
        os.makedirs(os.path.dirname(save_json) or ".", exist_ok=True)
        with open(save_json, "w") as f:
            json.dump(results, f, indent=2)
        print(f"\nSaved predictions to {save_json}")
...
@torch.no_grad()
def retrieval_report(cfg: TrainConfig, num_queries: int = 8, topk: int = 5, max_pairs_per_image: int = 2):
    device = torch.device(cfg.device)
    vocab = Vocab.from_files(cfg.classes_txt, cfg.attributes_yaml)
    df = pd.read_csv(cfg.labels_csv)
    df.columns = [str(c).strip().lower() for c in df.columns]

    # Ensure wide attributes exist
    needed = ["color","material","condition","size"]
    has_wide = all(k in df.columns for k in needed)
    if ("attributes" in df.columns) and (not has_wide):
        parsed = df["attributes"].apply(parse_attr_string)
    ...
    # Load model + retrieval artifacts
    run_dir = os.path.join(cfg.out_dir, os.path.basename(cfg.model_name))
    best_path = os.path.join(run_dir, "best.pt")
    rpath = os.path.join(run_dir, "retrieval.pt")
    assert os.path.exists(best_path) and os.path.exists(rpath), "Train classifier + retrieval first."

    state = torch.load(best_path, map_location="cpu")
    model = MultiTaskNet(cfg.model_name, vocab).to(device).eval()
    model.load_state_dict(state["model"])

    rstate = torch.load(rpath, map_location="cpu")
    txt = TextEncoder().to(device).eval()
    txt.load_state_dict(rstate["text_encoder"])  # type: ignore
    head = RetrievalHead(model.backbone.num_features, txt.dim, 256).to(device).eval()
    head.load_state_dict(rstate["retrieval_head"])  # type: ignore
    ...
    for p in tqdm(full_paths, desc="encode-gallery", leave=False):
        ...
        gfeats.append(model.extract_features(x))
    gfeats = torch.cat(gfeats, 0)

    # Build queries from rows, sample some
    phrases = []
    for _, row in df.iterrows():
        phrases.extend([(build_phrases_from_row(row, vocab)[0], row["path"])])  # one phrase per image
    random.seed(42)
    random.shuffle(phrases)
    phrases = phrases[:num_queries]

    # Evaluate & save galleries
    wrong_rows = []
    for qi, (q, src_path) in enumerate(phrases, 1):
        qz = txt.encode([q], cfg.device)
        zi, zt, _ = head(gfeats, qz)
        scores = (zi @ zt.t()).squeeze(1).cpu().numpy()
        top_idx = np.argsort(-scores)[:topk]
        top_paths = [full_paths[i] for i in top_idx]
        titles = [f"#{i+1} {os.path.basename(top_paths[i])}" for i in range(len(top_paths))]
        out_png = os.path.join(run_dir, f"retrieval_{qi:02d}.png")
        save_image_grid(top_paths, titles, out_png, cols=topk)

        # mark wrong if the source image isn't in top-k
        src_full = src_path if os.path.isabs(src_path) else os.path.join(cfg.data_root, src_path)
        hit = src_full in top_paths
        if not hit:
            wrong_rows.append({"query": q, "source": src_full, "topk": ";".join(top_paths)})

    if wrong_rows:
        pd.DataFrame(wrong_rows).to_csv(os.path.join(run_dir, "retrieval_wrong.csv"), index=False)
    print(f"[retrieval report] saved galleries and retrieval_wrong.csv (if any) to {run_dir}")

# -----------------------------
# CLI
# -----------------------------
MODELS = {
    "tinyvit": "tiny_vit_11m_224",                # (you can keep it, but I'd avoid using it)
    "swint": "swin_tiny_patch4_window7_224",
    "vits": "vit_small_patch16_224",              # ✅ good TinyViT replacement
    "convnext_tiny": "convnext_tiny.fb_in22k_ft_in1k"  # or "convnext_tiny"
}
...
def main():
    ...
    p_pred.add_argument("--image", nargs='+', required=True, help="Path(s) or glob(s) to image files")
    p_pred.add_argument("--topk", type=int, default=5)
    p_pred.add_argument("--save_json", default=None)
    ...
    p_report.add_argument("--split", choices=["train","val","test"], default="val")
    p_report.add_argument("--max_mis", type=int, default=16)
    p_report.add_argument("--no_tsne", action="store_true")
    p_report.add_argument("--no_pca", action="store_true")

    p_retreval = sub.add_parser("retrieval_report", parents=[common])
    p_retreval.add_argument("--num_queries", type=int, default=8)
    p_retreval.add_argument("--topk", type=int, default=5)
    ...
    if args.cmd == "train":
        ...
        if not paths:
            ...
            predict_cmd(cfg, paths, topk=args.topk, save_json=args.save_json)
    elif args.cmd == "report":
        eval_report(cfg, split=args.split, max_mis=args.max_mis,
                    do_tsne=not args.no_tsne, do_pca=not args.no_pca)
    elif args.cmd == "retrieval_report":
        retrieval_report(cfg, num_queries=args.num_queries, topk=args.topk)
